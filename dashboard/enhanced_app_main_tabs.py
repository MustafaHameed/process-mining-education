...
# Import dashboard components (use fixed versions)
from components.process_map_fixed import generate_process_map
...
# Import interpreters (use relative imports to match workspace layout)
...
<style>
    ...
    /* Style to emphasize main content and tabs */
...
def main():
    # Main header in the main content area
    ...
    # Create a 2-column layout with sidebar
    # Left sidebar for configuration
    with st.sidebar:
        st.header("Configuration")
        
        # Dataset selection tabs in sidebar
                    ...
                    if st.button("Process EPM Dataset", key="process_epm"):
                        with st.spinner("Loading and processing EPM dataset..."):
                            ...
                            if raw_data.empty:
                                ...
                                st.session_state['event_log'] = quality_log
                ...
                if uploaded_file.name.endswith('.csv'):
                    ...
                    if st.button("Process Event Log", key="process_xes"):
                        try:
                            ...
                            st.error(f"Error reading XES file: {str(e)}")
    ...
    if 'data_loaded' in st.session_state and st.session_state['data_loaded']:
        ...
        display_dashboard(event_log, raw_data)
    else:
        # Show welcome message if no data is loaded
        st.info("👈 Please use the sidebar to configure and load a dataset.")
        
        # Show example image or demo
        st.subheader("Process Mining Dashboard Overview")
        st.markdown("""
        This dashboard allows you to analyze educational process data using process mining techniques.
        
        **Available analyses:**
        
        1. **Process Map** - Visualize the flow of activities
        2. **Performance Metrics** - Analyze process efficiency
        3. **Patterns & Insights** - Discover recurring patterns
        4. **Conformance Checking** - Compare actual vs. expected processes
        
        To get started, select a dataset in the sidebar.
        """)
...
def display_dashboard(event_log, raw_data=None):
    ...
    # Activity frequency and hourly distribution charts are shown inside the Overview tab only
